...
homepage: https://github.com/acidkill/surreal-memory
metadata: {"openclaw":{"emoji":"brain","primaryEnv":"SURREAL_MEMORY_BRAIN","requires":{"bins":["python3"],"env":["SURREAL_MEMORY_BRAIN"]},"os":["darwin","linux","win32"],"install":[{"id":"pip","kind":"node","package":"surreal-memory","bins":["smem"],"label":"pip install surreal-memory"}]}}
---

# Surreal-Memory — Associative Memory for AI Agents
...
**Why not just vector search?** Vector search finds documents similar to your query. Surreal-Memory finds *conceptually related* memories through graph traversal — even when there's no keyword or embedding overlap. "What decision did we make about auth?" activates time + entity + concept neurons simultaneously and finds the intersection.
...
The brain and config at `~/.surrealmemory/` are auto-created on first use.
...
**Plugin features:**
- 6 tools registered automatically (smem_remember, smem_recall, smem_context, smem_todo, smem_stats, smem_health)
...
```json
{
  "plugins": {
    "load": {
      "paths": ["C:/Users/<you>/AppData/Roaming/npm/node_modules/surrealmemory"]
    }
  }
}
```
...
```powershell
npm list -g surrealmemory --parseable
```
...
| Agent runs `smem remember` in terminal | Agent confused CLI vs tool | Plugin now auto-injects tool instructions via `systemPrompt` |
...
|------|---------|-------------|
| `smem_remember` | Store a memory | After decisions, errors, facts, insights, user preferences |
| `smem_recall` | Query memories | Before tasks, when user references past context, "do you remember..." |
| `smem_context` | Get recent memories | At session start, inject fresh context |
| `smem_todo` | Quick TODO with 30-day expiry | Task tracking |

### Intelligence Tools

| Tool | Purpose | When to Use |
|------|---------|-------------|
| `smem_auto` | Auto-extract memories from text | After important conversations — captures decisions, errors, TODOs automatically |
| `smem_recall` (depth=3) | Deep associative recall | Complex questions requiring cross-domain connections |
| `smem_habits` | Workflow pattern suggestions | When user repeats similar action sequences |

### Management Tools

| Tool | Purpose | When to Use |
|------|---------|-------------|
| `smem_health` | Brain health diagnostics | Periodic checkup, before sharing brain |
| `smem_stats` | Brain statistics | Quick overview of memory counts |
| `smem_version` | Brain snapshots and rollback | Before risky operations, version checkpoints |
| `smem_transplant` | Transfer memories between brains | Cross-project knowledge sharing |

## Workflow

### At Session Start
1. Call `smem_context` to inject recent memories into your awareness
2. If user mentions a specific topic, call `smem_recall` with that topic

### During Conversation
3. When a decision is made: `smem_remember` with type="decision"
4. When an error occurs: `smem_remember` with type="error"
5. When user states a preference: `smem_remember` with type="preference"
6. When asked about past events: `smem_recall` with appropriate depth

### At Session End
7. Call `smem_auto` with action="process" on important conversation segments
