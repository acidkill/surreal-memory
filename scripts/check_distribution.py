#!/usr/bin/env python3
"""Check all distribution channels for Surreal-Memory and report version mismatches.
...
PACKAGE_NAME = "surreal-memory"
NPM_PACKAGE = "surrealmemory"
VSCE_ID = "neuralmem.surrealmemory"
CLAWHUB_SLUG = "surreal-memory"
GITHUB_REPO = "nhadaututtheky/surreal-memory"
...
ALL_VERSION_FILES: list[str] = [
    "pyproject.toml",
    "src/surreal_memory/__init__.py",
...
def check_local_init() -> ChannelResult:
    """Read __version__ from src/surreal_memory/__init__.py."""
    path = ROOT / "src" / "surreal_memory" / "__init__.py"
    try:
...
def _fix_hints(channel: ChannelResult, local_version: str) -> list[str]:
    ...
    if name == "PyPI":
        return [
            "# Publish to PyPI:",
            f"python -m build",
            f"twine upload dist/surreal_memory-{local_version}*",
        ]
    if name == "npm":
        return [
            "# Publish to npm:",
            f"cd integrations/surrealmemory",
    ...
    if "__init__" in name.lower():
        return [f'# Update src/surreal_memory/__init__.py: __version__ = "{local_version}"']
...
def main() -> int:
    ...
    print(f"{_BOLD}  Surreal-Memory - Distribution Channel Check{_RESET}")
    print(f"{_BOLD}{'=' * 62}{_RESET}")
