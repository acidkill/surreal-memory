"""Benchmark: Mem0 vs Surreal-Memory on real-world memory tasks.
...
Run:
    SURREALDB_URL=ws://localhost:8001/rpc python scripts/benchmark_mem0_vs_nm.py
...
for _name in [
    "surreal_memory.engine",
    "surreal_memory.storage",
    "surreal_memory.safety",
    "surreal_memory.extraction",
    "httpx",
    "httpcore",
    "openai",
    "litellm",
...
MEMORIES_50: list[str] = [
    ...
    "Neuron types in Surreal-Memory: FACT, DECISION, ERROR, WORKFLOW, CONCEPT, ENTITY, PATTERN.",
    ...
    "Surreal-Memory project is maintained at nhadaututtheky/surreal-memory on GitHub.",
    "Alex is the tech lead; Maria is the product manager; Chen is the senior backend engineer.",
    "The sync hub runs on Cloudflare Workers + D1 at surreal-memory-sync-hub.vietnam11399.workers.dev.",
    "Qwen-Plus is Alibaba Cloud's flagship LLM, accessible via DashScope API with litellm prefix.",
    "The Surreal-Memory MCP server exposes 44 tools and communicates via stdio transport.",
...
QUERIES_20: list[str] = [
    ...
    "How does Surreal-Memory retrieve memories?",
    ...
    "What neuron types exist in Surreal-Memory?",
...
MULTIHOP_QUERIES: list[tuple[str, list[str]]] = [
    ...
    (
        "What are the key Surreal-Memory engine classes and how do they interact?",
...
# ---------------------------------------------------------------------------
# Surreal-Memory helpers
# ---------------------------------------------------------------------------


async def nm_setup(brain_id: str) -> tuple[Any, Any, Any]:
    """Create + initialise SurrealDB storage, Brain, Encoder, ReflexPipeline."""
...
def print_report(suite: BenchmarkSuite) -> None:
    ...
    print(f"\n{sep}")
    print("  BENCHMARK RESULTS: Surreal-Memory vs Mem0")
    print(sep)
    ...
    print(f"\n  API CALL COST")
    print(thin)
    print(f"  Surreal-Memory total external API calls : {suite.nm_total_api_calls}")
    ...
    if not suite.mem0_available:
        ...
        print(f"  Surreal-Memory wins : {nm_wins} categories")
        print(f"  Mem0 wins         : {mem0_wins} categories")
        if nm_wins > mem0_wins:
            print("  >> Surreal-Memory outperforms Mem0 overall in this benchmark.")
        elif mem0_wins > nm_wins:
            print("  >> Mem0 outperforms Surreal-Memory overall in this benchmark.")
        else:
...
async def main() -> None:
    print("=" * 84)
    print("  Surreal-Memory vs Mem0 — Real-World Memory Benchmark")
    print("=" * 84)
    ...
    if not surrealdb_url:
        ...
        print("    SURREALDB_URL=ws://localhost:8001/rpc python scripts/benchmark_mem0_vs_nm.py")
        sys.exit(1)

    suite = BenchmarkSuite()

    # Temp dir (Mem0's local Qdrant store only -- NM uses SurrealDB directly)
    tmp_dir = Path(tempfile.mkdtemp(prefix="smem_bench_"))
    nm_brain_id = f"bench-mem0-{uuid.uuid4().hex[:8]}"
