"""Tests for smem init --full (extended setup)."""
...
from surreal_memory.cli.full_setup import (
    _PROVIDER_PRIORITY,
    QUICKSTART_URL,
...
class TestDetectEmbeddingProvider:
    ...
    def test_detects_sentence_transformers(self) -> None:
        with patch(
            "surreal_memory.cli.full_setup._is_module_available",
...
class TestGenerateMaintenanceScript:
    ...
    def test_creates_bash_script_on_unix(self, tmp_path: Path) -> None:
        with patch("surreal_memory.cli.full_setup.os.name", "posix"):
            result = generate_maintenance_script(tmp_path)
            assert result is not None
            assert result.name == "maintenance.sh"
            assert result.exists()
            content = result.read_text(encoding="utf-8")
            assert "smem decay" in content
            assert "smem consolidate" in content
            assert "smem doctor" in content

    def test_creates_powershell_on_windows(self, tmp_path: Path) -> None:
        with patch("surreal_memory.cli.full_setup.os.name", "nt"):
            result = generate_maintenance_script(tmp_path)
            assert result is not None
...
class TestRunFullSetup:
    ...
    @patch("surreal_memory.cli.full_setup.generate_maintenance_script", return_value=None)
    @patch(
        "surreal_memory.cli.full_setup.enable_config_defaults", return_value={"dedup": "enabled"}
    )
    @patch("surreal_memory.cli.full_setup.detect_embedding_provider", return_value=None)
    @patch("surreal_memory.cli.full_setup._prompt_install_embeddings", return_value=None)
    @patch("surreal_memory.cli.setup.setup_skills", return_value={"memory-intake": "installed"})
    @patch("surreal_memory.cli.setup.setup_hooks_claude", return_value="added")
    @patch("surreal_memory.cli.setup.setup_mcp_cursor", return_value="not_found")
    @patch("surreal_memory.cli.setup.setup_mcp_claude", return_value="added")
    @patch("surreal_memory.cli.setup.setup_brain", return_value="default")
    @patch("surreal_memory.cli.setup.setup_config", return_value=True)
    @patch("surreal_memory.unified_config.get_surrealmemory_dir", return_value=Path("/tmp/nm"))
    @patch("surreal_memory.cli.setup.print_summary")
    @patch("surreal_memory.cli.full_setup.print_full_banner")
    def test_full_flow(
        self,
    ...
    @patch("surreal_memory.cli.full_setup.generate_maintenance_script", return_value=None)
    @patch("surreal_memory.cli.full_setup.enable_config_defaults", return_value={})
    @patch("surreal_memory.cli.full_setup.detect_embedding_provider")
    @patch("surreal_memory.cli.setup.setup_skills", return_value={"s1": "exists"})
    @patch("surreal_memory.cli.setup.setup_hooks_claude", return_value="exists")
    @patch("surreal_memory.cli.setup.setup_mcp_cursor", return_value="not_found")
    @patch("surreal_memory.cli.setup.setup_mcp_claude", return_value="exists")
    @patch("surreal_memory.cli.setup.setup_brain", return_value="default")
    @patch("surreal_memory.cli.setup.setup_config", return_value=False)
    @patch("surreal_memory.unified_config.get_surrealmemory_dir", return_value=Path("/tmp/nm"))
    @patch("surreal_memory.cli.setup.print_summary")
    @patch("surreal_memory.cli.full_setup.print_full_banner")
    def test_skip_mcp(
...
class TestIsModuleAvailable:
    ...
    def test_handles_import_error(self) -> None:
        ...
        with patch(
            "surreal_memory.cli.full_setup.importlib.util.find_spec",
...
class TestRunFullSetupSkipEmbeddings:
    ...
    @patch("surreal_memory.cli.full_setup.enable_config_defaults", return_value={})
    @patch("surreal_memory.cli.full_setup.detect_embedding_provider", return_value=None)
    @patch("surreal_memory.cli.setup.setup_skills", return_value={"s1": "installed"})
    @patch("surreal_memory.cli.setup.setup_hooks_claude", return_value="added")
    @patch("surreal_memory.cli.setup.setup_mcp_cursor", return_value="not_found")
    @patch("surreal_memory.cli.setup.setup_mcp_claude", return_value="added")
    @patch("surreal_memory.cli.setup.setup_brain", return_value="default")
