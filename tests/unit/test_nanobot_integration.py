...
class TestToolSchemas:
    ...
    def test_health_schema(self, nm_context: NMContext) -> None:
        ...
        assert tool.name == "smem_health"
        schema = tool.to_schema()
        assert schema["function"]["name"] == "smem_health"
...
class TestRecallTool:
    ...
    async def test_recall_after_remember(self, nm_context: NMContext) -> None:
        from surreal_memory.integrations.nanobot.tools import NMRecallTool, NMRememberTool
...
class TestContextTool:
    ...
    async def test_context_with_memories(self, nm_context: NMContext) -> None:
        from surreal_memory.integrations.nanobot.tools import NMContextTool, NMRememberTool
...
class TestHealthTool:
    """Test smem_health tool execution."""
...
class TestNMMemoryStore:
    ...
    async def test_get_memory_context_with_data(self, store: Any) -> None:
        ...
        if result:
            assert "Surreal-Memory" in result or "Recent" in result
...
class TestSetup:
    """Test setup_surreal_memory entry point."""
    ...
    async def test_setup_registers_correct_tools(self, workspace: Path) -> None:
        ...
        mock_registry = MagicMock()
        store = await setup_surreal_memory(mock_registry, workspace)

        registered_names = {call.args[0].name for call in mock_registry.register.call_args_list}
        assert registered_names == {"smem_remember", "smem_recall", "smem_context", "smem_health"}
    ...
    async def test_setup_idempotent(self, workspace: Path) -> None:
        ...
        mock_registry = MagicMock()
        store1 = await setup_surreal_memory(mock_registry, workspace, brain_id="idem")
        brain_id_1 = store1._ctx.brain.id
        await store1._ctx.close()

        store2 = await setup_surreal_memory(mock_registry, workspace, brain_id="idem")
