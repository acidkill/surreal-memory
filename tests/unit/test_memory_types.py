...
class TestSuggestMemoryType:
    ...
    def test_suggests_instruction(self) -> None:
        ...
        "Never use X" now classifies as BOUNDARY (safety rule) per the
        precedence rule that BOUNDARY wins over INSTRUCTION. Use a
        non-safety phrasing here ("avoid using eval") to keep the
        INSTRUCTION assertion meaningful.
        """
    ...
    def test_fact_not_misclassified_as_decision(self) -> None:
        ...
        assert (
            suggest_memory_type("Config file located at ~/.surrealmemory/config.toml")
            == MemoryType.FACT
        )
