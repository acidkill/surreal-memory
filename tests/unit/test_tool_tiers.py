...
from surreal_memory.mcp.tool_schemas import (
    TOOL_TIERS,
    get_tool_schemas,
...
class TestToolTiers:
    ...
    def test_explain_not_in_standard_or_minimal(self) -> None:
        assert "smem_explain" not in TOOL_TIERS["standard"]
        assert "smem_explain" not in TOOL_TIERS["minimal"]
...
class TestServerTierIntegration:
    ...
    def _make_server(self, tier: str) -> MCPServer:  # noqa: F821
        ...
        with patch("surreal_memory.mcp.server.get_config") as mock:
    ...
    def test_server_full_tier(self) -> None:
        ...
        with patch("surreal_memory.plugins.get_plugin_tools", return_value=[]):
            assert len(server.get_tools()) == 58
    ...
    async def test_hidden_tools_still_callable(self) -> None:
        ...
        assert "smem_stats" not in exposed_names
