...
from surreal_memory.engine.token_budget import (
    BudgetAllocation,
    BudgetConfig,
    TokenCost,
    allocate_budget,
...
class TestFormatContextBudgeted:
    ...
    async def test_budget_allocation_returned(self, mock_storage: AsyncMock) -> None:
        from surreal_memory.engine.retrieval_context import format_context_budgeted
        from surreal_memory.engine.token_budget import BudgetAllocation
...
class TestNmemBudget:
    """Tests for the smem_budget MCP tool handler."""

    @pytest.fixture
    def server(self) -> Any:
        """Create minimal MCPServer mock."""
        from surreal_memory.mcp.server import MCPServer
        from surreal_memory.unified_config import BudgetRetrievalConfig, ToolTierConfig

        with patch("surreal_memory.mcp.server.get_config") as mock_cfg:
...
class TestNmemRecallWithBudget:
    """Tests for smem_recall with recall_token_budget parameter."""
