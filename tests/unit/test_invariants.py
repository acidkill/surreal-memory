...
class TestE2EIsolation:
    """Verify E2E tests use isolated storage via SURREAL_MEMORY_DIR."""
    ...
    def test_e2e_client_fixture_uses_tmp_path(self) -> None:
        ...
        assert "monkeypatch" in params, (
            "E2E client fixture must use monkeypatch to override SURREAL_MEMORY_DIR"
        )

    def test_surrealmemory_dir_env_respected(self, tmp_path: object) -> None:
        """get_surrealmemory_dir() must respect SURREAL_MEMORY_DIR env var."""
        import os
        from pathlib import Path

        from surreal_memory.unified_config import get_surrealmemory_dir

        original = os.environ.get("SURREAL_MEMORY_DIR")
        try:
            test_dir = str(tmp_path)
            os.environ["SURREAL_MEMORY_DIR"] = test_dir
            result = get_surrealmemory_dir()
            assert result == Path(test_dir).resolve()
        finally:
            if original is None:
                os.environ.pop("SURREAL_MEMORY_DIR", None)
            else:
                os.environ["SURREAL_MEMORY_DIR"] = original
