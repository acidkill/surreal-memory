...
from surreal_memory.safety.input_firewall import (
    FirewallResult,
    _char_entropy,
...
class TestNmContextNoiseStripping:
    """Tests for Surreal-Memory context noise removal (issue #118)."""
    ...
    def test_strip_nm_wrapper_line(self) -> None:
        text = "[Surreal-Memory — relevant context]\nSome actual content that the user typed for this memory storage."
        result = check_content(text)
        assert result.blocked is False
        assert "[Surreal-Memory" not in result.sanitized
    ...
    def test_strip_nm_context_noise_function(self) -> None:
        ...
        assert "[Surreal-Memory" not in cleaned
...
class TestSanitizeExplicitContent:
    """Tests for sanitize_explicit_content() — explicit smem_remember path (ADR-001)."""
    ...
    def test_strips_nm_wrapper(self) -> None:
        text = "[Surreal-Memory — context]\nReal user content for storage."
        result = sanitize_explicit_content(text)
        assert "[Surreal-Memory" not in result
