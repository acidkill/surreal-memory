...
from surreal_memory.mcp.version_check_handler import (
    VersionInfo,
...
class TestGetUpdateHint:
    ...
    def test_normal_hint_for_pip_install(self) -> None:
        ...
        with patch(
            "surreal_memory.mcp.version_check_handler._is_editable_install", return_value=False
        ):
    ...
    def test_editable_hint_for_dev_install(self) -> None:
        handler = _make_handler(update_available=True)
        with patch(
            "surreal_memory.mcp.version_check_handler._is_editable_install", return_value=True
        ):
            hint = handler.get_update_hint()
        assert hint is not None
