...
class TestMCPServer:
    ...
    def test_remember_tool_schema(self, server: MCPServer) -> None:
        """Test smem_remember tool schema."""
        tools = server.get_tools()
        remember_tool = next(t for t in tools if t["name"] == "smem_remember")
    ...
    def test_recall_tool_schema(self, server: MCPServer) -> None:
        """Test smem_recall tool schema."""
        tools = server.get_tools()
        recall_tool = next(t for t in tools if t["name"] == "smem_recall")
    ...
    def test_context_tool_schema(self, server: MCPServer) -> None:
        """Test smem_context tool schema."""
        tools = server.get_tools()
        context_tool = next(t for t in tools if t["name"] == "smem_context")
    ...
    def test_todo_tool_schema(self, server: MCPServer) -> None:
        """Test smem_todo tool schema."""
        tools = server.get_tools()
        todo_tool = next(t for t in tools if t["name"] == "smem_todo")
    ...
    def test_stats_tool_schema(self, server: MCPServer) -> None:
        """Test smem_stats tool schema."""
        tools = server.get_tools()
        stats_tool = next(t for t in tools if t["name"] == "smem_stats")
    ...
    def test_suggest_tool_schema(self, server: MCPServer) -> None:
        """Test smem_suggest tool schema."""
        tools = server.get_tools()
        suggest_tool = next(t for t in tools if t["name"] == "smem_suggest")
    ...
    def test_session_tool_schema(self, server: MCPServer) -> None:
        """Test smem_session tool schema."""
        tools = server.get_tools()
        session_tool = next(t for t in tools if t["name"] == "smem_session")
    ...
    def test_index_tool_schema(self, server: MCPServer) -> None:
        """Test smem_index tool schema."""
        tools = server.get_tools()
        index_tool = next(t for t in tools if t["name"] == "smem_index")
...
class TestMCPToolCalls:
    ...
    async def test_remember_tool(self, server: MCPServer) -> None:
        """Test smem_remember tool execution."""
        ...
        with (
            ...
            patch("surreal_memory.mcp.remember_handler.MemoryEncoder", return_value=mock_encoder),
        ):
    ...
    async def test_remember_no_brain(self, server: MCPServer) -> None:
        """Test smem_remember when no brain is configured."""
        mock_storage = AsyncMock()
        ...
        with patch.object(server, "get_storage", return_value=mock_storage):
            result = await server.call_tool("smem_remember", {"content": "Test"})
    ...
    async def test_recall_tool(self, server: MCPServer) -> None:
        """Test smem_recall tool execution."""
        mock_storage = AsyncMock()
        mock_brain = MagicMock(
            id="test-brain",
            name="test",
        ...
        with (
            ...
            result = await server.call_tool("smem_recall", {"query": "test query"})
    ...
    async def test_recall_low_confidence(self, server: MCPServer) -> None:
        """Test smem_recall with confidence below threshold."""
        mock_storage = AsyncMock()
        mock_brain = MagicMock(
            id="test-brain",
            name="test",
        ...
        with (
            ...
            result = await server.call_tool("smem_recall", {"query": "test", "min_confidence": 0.5})
    ...
    async def test_context_tool(self, server: MCPServer) -> None:
        """Test smem_context tool execution."""
        mock_storage = AsyncMock()
        mock_fibers = [
            MagicMock(
                summary="Memory 1",
        ...
        with patch.object(server, "get_storage", return_value=mock_storage):
            result = await server.call_tool("smem_context", {"limit": 5})
    ...
    async def test_context_empty(self, server: MCPServer) -> None:
        """Test smem_context with no memories."""
        mock_storage = AsyncMock()
        ...
        with patch.object(server, "get_storage", return_value=mock_storage):
            result = await server.call_tool("smem_context", {})
    ...
    async def test_todo_tool(self, server: MCPServer) -> None:
        """Test smem_todo tool (delegates to remember)."""
        mock_storage = AsyncMock()
        mock_brain = MagicMock(
            id="test-brain",
            name="test",
        ...
        with (
            ...
            result = await server.call_tool("smem_todo", {"task": "Review code", "priority": 8})
    ...
    async def test_stats_tool(self, server: MCPServer) -> None:
        """Test smem_stats tool execution."""
        mock_storage = AsyncMock()
        mock_brain = MagicMock()
        ...
        with patch.object(server, "get_storage", return_value=mock_storage):
            result = await server.call_tool("smem_stats", {})
    ...
    async def test_auto_tool_status(self, server: MCPServer) -> None:
        """Test smem_auto status action."""
        result = await server.call_tool("smem_auto", {"action": "status"})
    ...
    async def test_auto_tool_analyze(self, server: MCPServer) -> None:
        """Test smem_auto analyze action."""
        text = "We decided to use PostgreSQL for the database. TODO: Set up migrations."
        result = await server.call_tool("smem_auto", {"action": "analyze", "text": text})
    ...
    async def test_auto_tool_analyze_errors(self, server: MCPServer) -> None:
        """Test smem_auto detects error patterns."""
    ...
    async def test_auto_tool_analyze_empty(self, server: MCPServer) -> None:
        """Test smem_auto with no detectable content."""
        result = await server.call_tool("smem_auto", {"action": "analyze", "text": "Hello world"})

        assert result["detected"] == []

    @pytest.mark.asyncio
    async def test_auto_tool_process(self) -> None:
        """Test smem_auto process action (analyze + save)."""
        ...
        with (
            ...
            result = await server.call_tool("smem_auto", {"action": "process", "text": text})
    ...
    async def test_auto_tool_process_empty(self, server: MCPServer) -> None:
        """Test smem_auto process with no detectable content."""
        result = await server.call_tool("smem_auto", {"action": "process", "text": "Hello world"})

        assert result["saved"] == 0

    @pytest.mark.asyncio
    async def test_suggest_basic(self, server: MCPServer) -> None:
        """Test smem_suggest returns matching suggestions."""
        mock_storage = AsyncMock()
        ...
        with patch.object(server, "get_storage", return_value=mock_storage):
            result = await server.call_tool("smem_suggest", {"prefix": "API"})
    ...
    async def test_suggest_empty_prefix(self, server: MCPServer) -> None:
        """Test smem_suggest with empty prefix returns idle neurons (reinforcement mode)."""
        mock_storage = AsyncMock()
        mock_storage.get_all_neuron_states = AsyncMock(return_value=[])

        with patch.object(server, "get_storage", return_value=mock_storage):
            result = await server.call_tool("smem_suggest", {"prefix": ""})
    ...
    async def test_suggest_with_type_filter(self, server: MCPServer) -> None:
        """Test smem_suggest with type_filter."""
        mock_storage = AsyncMock()
        mock_storage.suggest_neurons = AsyncMock(return_value=[])

        with patch.object(server, "get_storage", return_value=mock_storage):
            result = await server.call_tool(
                "smem_suggest", {"prefix": "auth", "type_filter": "concept"}
            )
    ...
    async def test_suggest_respects_limit(self, server: MCPServer) -> None:
        """Test smem_suggest respects limit parameter."""
        mock_storage = AsyncMock()
        mock_storage.suggest_neurons = AsyncMock(return_value=[])

        with patch.object(server, "get_storage", return_value=mock_storage):
            await server.call_tool("smem_suggest", {"prefix": "test", "limit": 2})
    ...
    async def test_index_scan(self, server: MCPServer) -> None:
        """Test smem_index scan action returns file/neuron counts."""
        mock_storage = AsyncMock()
        ...
        with (
            patch.object(server, "get_storage", return_value=mock_storage),
            patch(
                "surreal_memory.engine.codebase_encoder.CodebaseEncoder",
                return_value=mock_encoder,
            ),
        ):
            result = await server.call_tool("smem_index", {"action": "scan", "path": "."})
    ...
    async def test_index_status_empty(self, server: MCPServer) -> None:
        """Test smem_index status when nothing indexed."""
        mock_storage = AsyncMock()
        mock_storage.find_neurons = AsyncMock(return_value=[])

        with patch.object(server, "get_storage", return_value=mock_storage):
            result = await server.call_tool("smem_index", {"action": "status"})
    ...
    async def test_session_get_empty(self, server: MCPServer) -> None:
        """Test smem_session get with no active session."""
        mock_storage = AsyncMock()
        mock_storage.find_typed_memories = AsyncMock(return_value=[])

        with patch.object(server, "get_storage", return_value=mock_storage):
            result = await server.call_tool("smem_session", {"action": "get"})
    ...
    async def test_session_set_and_get(self, server: MCPServer) -> None:
        """Test smem_session set then get roundtrip."""
        mock_storage = AsyncMock()
        mock_brain = MagicMock(id="test-brain", name="test", config=MagicMock())
        mock_storage.get_brain = AsyncMock(return_value=mock_brain)
        mock_storage._current_brain_id = "test-brain"
    ...
    async def test_session_end(self, server: MCPServer) -> None:
        """Test smem_session end creates tombstone and summary."""
        mock_storage = AsyncMock()
        mock_brain = MagicMock(id="test-brain", name="test", config=MagicMock())
        mock_storage.get_brain = AsyncMock(return_value=mock_brain)
        mock_storage._current_brain_id = "test-brain"
        ...
        with (
            ...
            result = await server.call_tool("smem_session", {"action": "end"})
    ...
    async def test_session_end_no_active(self, server: MCPServer) -> None:
        """Test smem_session end with no active session."""
        mock_storage = AsyncMock()
        mock_storage.find_typed_memories = AsyncMock(return_value=[])
    ...
    async def test_consolidate_default(self, server: MCPServer) -> None:
        """Test smem_consolidate with default strategy."""
        mock_storage = AsyncMock()
        mock_storage._current_brain_id = "test-brain"
        mock_storage.brain_id = "test-brain"
    ...
    async def test_consolidate_dry_run(self, server: MCPServer) -> None:
        ...
        with (
            ...
            result = await server.call_tool(
                "smem_consolidate", {"strategy": "prune", "dry_run": True}
            )
    ...
    async def test_consolidate_invalid_strategy(self, server: MCPServer) -> None:
        ...
        with patch.object(server, "get_storage", return_value=mock_storage):
            result = await server.call_tool("smem_consolidate", {"strategy": "invalid_strategy"})
    ...
    async def test_consolidate_no_brain(self, server: MCPServer) -> None:
        """Test smem_consolidate when no brain is configured."""
        mock_storage = AsyncMock()
        mock_storage.get_brain = AsyncMock(return_value=None)
...
class TestMCPErrorPaths:
    ...
    async def test_session_set_no_brain(self) -> None:
        """Test smem_session set when brain is missing."""
    ...
    async def test_session_end_no_brain(self) -> None:
        """Test smem_session end when brain is missing."""
        server = self._make_server()
        mock_storage = AsyncMock()
        mock_storage.get_brain = AsyncMock(return_value=None)
        mock_storage._current_brain_id = "test-brain"
    ...
    async def test_eternal_save_no_brain(self) -> None:
        """Test smem_eternal save when brain is missing returns error."""
        server = self._make_server()
        mock_storage = AsyncMock()
        mock_storage.get_brain = AsyncMock(return_value=None)
        mock_storage._current_brain_id = "test-brain"
    ...
    async def test_recap_topic_no_brain(self) -> None:
        """Test smem_recap with topic when brain is missing."""
        server = self._make_server()
        ctx = AsyncMock()
        ...
        with (
            ...
            result = await server.call_tool("smem_recap", {"topic": "auth"})
...
class TestMCPProtocol:
    ...
    async def test_initialize_message(self, server: MCPServer) -> None:
        ...
        assert response["result"]["serverInfo"]["name"] == "surreal-memory"
    ...
    async def test_tools_call_message(self, server: MCPServer) -> None:
        ...
        with patch.object(server, "get_storage", return_value=mock_storage):
            message = {
                ...
                "id": 3,
                "method": "tools/call",
                "params": {"name": "smem_context", "arguments": {"limit": 5}},
            }
    ...
    async def test_tools_call_plain_string_arguments(self, server: MCPServer) -> None:
        ...
        with patch.object(server, "call_tool", new_callable=AsyncMock) as mock_call:
            ...
            message = {
                ...
                "id": 6,
                "method": "tools/call",
                "params": {"name": "smem_remember", "arguments": "some plain text"},
            }
...
class TestMCPResources:
    ...
    def test_get_resources(self, server: MCPServer) -> None:
        ...
        assert "surrealmemory://prompt/system" in uris
        assert "surrealmemory://prompt/compact" in uris

    def test_get_resource_content_system(self, server: MCPServer) -> None:
        """Test getting system prompt content."""
        content = server.get_resource_content("surrealmemory://prompt/system")

        assert content is not None
        assert "Surreal-Memory" in content
        assert "smem_remember" in content

    def test_get_resource_content_compact(self, server: MCPServer) -> None:
        """Test getting compact prompt content."""
        content = server.get_resource_content("surrealmemory://prompt/compact")
    ...
    def test_get_resource_content_unknown(self, server: MCPServer) -> None:
        ...
        content = server.get_resource_content("surrealmemory://unknown")
    ...
    async def test_resources_read_message(self, server: MCPServer) -> None:
        ...
        message = {
            ...
            "params": {"uri": "surrealmemory://prompt/system"},
        }
        ...
        assert response["result"]["contents"][0]["uri"] == "surrealmemory://prompt/system"
        assert "Surreal-Memory" in response["result"]["contents"][0]["text"]
    ...
    async def test_resources_read_not_found(self, server: MCPServer) -> None:
        ...
        message = {
            ...
            "id": 3,
            "method": "resources/read",
            "params": {"uri": "surrealmemory://unknown"},
        }
...
class TestMCPStorage:
    ...
    async def test_get_storage_caches_instance(self) -> None:
        ...
        with patch(
            "surreal_memory.mcp.server.get_shared_storage",
...
class TestPassiveCapture:
    ...
    async def test_passive_capture_on_long_recall(self) -> None:
        ...
        with (
            ...
            await server.call_tool("smem_recall", {"query": long_query})
    ...
    async def test_passive_capture_skipped_short_query(self) -> None:
        ...
        with (
            ...
            await server.call_tool("smem_recall", {"query": "auth setup"})
    ...
    async def test_passive_capture_error_swallowed(self) -> None:
        ...
        with (
            ...
            patch("surreal_memory.engine.retrieval.ReflexPipeline", return_value=mock_pipeline),
            patch(
                "surreal_memory.mcp.auto_handler.analyze_text_for_memories",
            ...
            result = await server.call_tool("smem_recall", {"query": long_query})
...
class TestMCPEternal:
    """Tests for smem_eternal and smem_recap tool calls."""
    ...
    async def test_eternal_status(self) -> None:
        """Test smem_eternal status returns memory counts and session."""
        server = self._make_server()
        ctx = self._mock_eternal_context()

        with patch.object(server, "get_eternal_context", return_value=ctx):
            result = await server.call_tool("smem_eternal", {"action": "status"})
    ...
    async def test_eternal_save_empty(self) -> None:
        """Test smem_eternal save with no fields returns no changes."""
        server = self._make_server()
        ctx = self._mock_eternal_context()
        mock_storage = AsyncMock()
        mock_brain = MagicMock(id="test-brain", config=MagicMock())
        ...
        with (
            ...
            result = await server.call_tool("smem_eternal", {"action": "save"})
    ...
    async def test_eternal_save_project_context(self) -> None:
        """Test smem_eternal save with project_name and tech_stack."""
        server = self._make_server()
        ctx = self._mock_eternal_context()
        mock_storage = AsyncMock()
        mock_brain = MagicMock(id="test-brain", config=MagicMock())
    ...
    async def test_eternal_save_decision_and_instruction(self) -> None:
        """Test smem_eternal save with decision and instruction."""
        server = self._make_server()
        ctx = self._mock_eternal_context()
        mock_storage = AsyncMock()
        mock_brain = MagicMock(id="test-brain", config=MagicMock())
    ...
    async def test_eternal_unknown_action(self) -> None:
        """Test smem_eternal with unknown action."""
        server = self._make_server()
        ctx = self._mock_eternal_context()

        with patch.object(server, "get_eternal_context", return_value=ctx):
            result = await server.call_tool("smem_eternal", {"action": "bogus"})

        assert "error" in result

    @pytest.mark.asyncio
    async def test_recap_level(self) -> None:
        """Test smem_recap with level."""
        server = self._make_server()
        ctx = self._mock_eternal_context()

        with patch.object(server, "get_eternal_context", return_value=ctx):
            result = await server.call_tool("smem_recap", {"level": 2})
    ...
    async def test_recap_default_level(self) -> None:
        """Test smem_recap defaults to level 1."""
        server = self._make_server()
        ctx = self._mock_eternal_context()

        with patch.object(server, "get_eternal_context", return_value=ctx):
            result = await server.call_tool("smem_recap", {})
    ...
    async def test_recap_level_clamped(self) -> None:
        """Test smem_recap level is clamped to 1-3."""
        server = self._make_server()
        ctx = self._mock_eternal_context()

        with patch.object(server, "get_eternal_context", return_value=ctx):
            result = await server.call_tool("smem_recap", {"level": 99})

        assert result["level"] == 3

    @pytest.mark.asyncio
    async def test_recap_with_feature_welcome(self) -> None:
        """Test smem_recap appends welcome when feature is set."""
        server = self._make_server()
        ctx = self._mock_eternal_context()

        with patch.object(server, "get_eternal_context", return_value=ctx):
            result = await server.call_tool("smem_recap", {"level": 1})

        assert "Welcome back!" in result["message"]

    @pytest.mark.asyncio
    async def test_recap_no_welcome_without_feature(self) -> None:
        """Test smem_recap no welcome when no feature set."""
        server = self._make_server()
        ctx = self._mock_eternal_context()
        ctx.get_status = AsyncMock(
            return_value={
    ...
    async def test_recap_topic(self) -> None:
        """Test smem_recap with topic search."""
        server = self._make_server()
        ctx = self._mock_eternal_context()
        ...
        with (
            patch.object(server, "get_eternal_context", return_value=ctx),
            patch.object(server, "get_storage", return_value=mock_storage),
            patch("surreal_memory.mcp.eternal_handler.ReflexPipeline", return_value=mock_pipeline),
        ):
            result = await server.call_tool("smem_recap", {"topic": "auth"})
...
class TestMCPImport:
    """Tests for smem_import tool calls."""
    ...
    async def test_import_success(self) -> None:
        """Test smem_import with successful sync."""
        server = self._make_server()
        mock_storage = AsyncMock()
        mock_brain = MagicMock(id="test-brain", config=MagicMock())
        mock_storage.get_brain = AsyncMock(return_value=mock_brain)
        ...
        with (
            patch.object(server, "get_storage", return_value=mock_storage),
            patch("surreal_memory.integration.adapters.get_adapter", return_value=mock_adapter),
            patch("surreal_memory.integration.sync_engine.SyncEngine", return_value=mock_engine),
        ):
            result = await server.call_tool(
                "smem_import",
    ...
    async def test_import_no_brain(self) -> None:
        """Test smem_import when no brain configured."""
        server = self._make_server()
        mock_storage = AsyncMock()
        mock_storage.get_brain = AsyncMock(return_value=None)
        mock_storage._current_brain_id = "test-brain"
        ...
        with patch.object(server, "get_storage", return_value=mock_storage):
            result = await server.call_tool("smem_import", {"source": "chromadb"})

        assert "error" in result

    @pytest.mark.asyncio
    async def test_import_no_source(self) -> None:
        """Test smem_import without source."""
        server = self._make_server()
        mock_storage = AsyncMock()
        mock_brain = MagicMock(id="test-brain", config=MagicMock())
        mock_storage.get_brain = AsyncMock(return_value=mock_brain)
        ...
        with patch.object(server, "get_storage", return_value=mock_storage):
            result = await server.call_tool("smem_import", {"source": ""})

        assert "error" in result

    @pytest.mark.asyncio
    async def test_import_adapter_not_found(self) -> None:
        """Test smem_import with unknown adapter."""
        server = self._make_server()
        mock_storage = AsyncMock()
        mock_brain = MagicMock(id="test-brain", config=MagicMock())
        mock_storage.get_brain = AsyncMock(return_value=mock_brain)
        ...
        with (
            patch.object(server, "get_storage", return_value=mock_storage),
            patch(
                "surreal_memory.integration.adapters.get_adapter",
                side_effect=ValueError("Unknown adapter"),
            ),
        ):
            result = await server.call_tool("smem_import", {"source": "unknown_system"})
    ...
    async def test_import_sync_failure(self) -> None:
        """Test smem_import when sync raises."""
        server = self._make_server()
        mock_storage = AsyncMock()
        mock_brain = MagicMock(id="test-brain", config=MagicMock())
        mock_storage.get_brain = AsyncMock(return_value=mock_brain)
    ...
    async def test_import_adapter_kwargs_awf(self, tmp_path: object) -> None:
        ...
        with (
            patch.object(server, "get_storage", return_value=mock_storage),
            patch("surreal_memory.integration.adapters.get_adapter") as mock_get_adapter,
            patch("surreal_memory.integration.sync_engine.SyncEngine", return_value=mock_engine),
        ):
            mock_get_adapter.return_value = MagicMock()
            await server.call_tool("smem_import", {"source": "awf", "connection": brain_dir})
...
class TestMCPAutoExtended:
    """Extended tests for smem_auto enable/disable/error paths."""
    ...
    async def test_auto_enable(self) -> None:
        """Test smem_auto enable action toggles enabled flag."""
        server = self._make_server(auto_enabled=False)
        ...
        with patch("dataclasses.replace", side_effect=fake_replace):
            result = await server.call_tool("smem_auto", {"action": "enable"})
    ...
    async def test_auto_disable(self) -> None:
        """Test smem_auto disable action toggles enabled flag."""
        server = self._make_server(auto_enabled=True)
        ...
        with patch("dataclasses.replace", side_effect=fake_replace):
            result = await server.call_tool("smem_auto", {"action": "disable"})
    ...
    async def test_auto_analyze_empty_text(self) -> None:
        """Test smem_auto analyze with empty text."""
        server = self._make_server()

        result = await server.call_tool("smem_auto", {"action": "analyze", "text": ""})

        assert "error" in result

    @pytest.mark.asyncio
    async def test_auto_process_empty_text(self) -> None:
        """Test smem_auto process with empty text."""
        server = self._make_server()

        result = await server.call_tool("smem_auto", {"action": "process", "text": ""})

        assert "error" in result

    @pytest.mark.asyncio
    async def test_auto_unknown_action(self) -> None:
        """Test smem_auto with unknown action."""
        server = self._make_server()

        result = await server.call_tool("smem_auto", {"action": "bogus"})

        assert "error" in result

    @pytest.mark.asyncio
    async def test_auto_analyze_with_save(self) -> None:
        """Test smem_auto analyze with save=True."""
        server = self._make_server()
        mock_storage = AsyncMock()
        mock_brain = MagicMock(id="test-brain", config=MagicMock())
        mock_storage.get_brain = AsyncMock(return_value=mock_brain)
...
class TestMCPContextExtended:
    """Extended tests for smem_context fresh_only and anchor fallback."""
    ...
    async def test_context_fresh_only(self, server: MCPServer) -> None:
        """Test smem_context with fresh_only=True filters old fibers."""
        ...
        with patch.object(server, "get_storage", return_value=mock_storage):
            result = await server.call_tool("smem_context", {"fresh_only": True, "limit": 10})
    ...
    async def test_context_anchor_fallback(self, server: MCPServer) -> None:
        """Test smem_context falls back to anchor neuron when fiber has no summary."""
        from datetime import datetime, timedelta
    ...
    async def test_context_no_summary_no_anchor(self, server: MCPServer) -> None:
        """Test smem_context with fiber that has no summary and no anchor."""
        mock_storage = AsyncMock()
        mock_storage.brain_id = None
        fiber = MagicMock(
            id="fiber-empty",
...
class TestMCPRecallExtended:
    ...
    async def test_recall_no_brain(self) -> None:
        """Test smem_recall when no brain configured."""
        server = self._make_server()
        mock_storage = AsyncMock()
        mock_storage.get_brain = AsyncMock(return_value=None)
        mock_storage._current_brain_id = "test-brain"
        ...
        with patch.object(server, "get_storage", return_value=mock_storage):
            result = await server.call_tool("smem_recall", {"query": "test"})
    ...
    async def test_recall_session_context_injection(self) -> None:
        ...
        with (
            ...
            result = await server.call_tool("smem_recall", {"query": "how it works"})
    ...
    async def test_remember_sensitive_content(self) -> None:
        """Test smem_remember handles sensitive content.
    ...
    async def test_remember_auto_type_detection(self) -> None:
        """Test smem_remember auto-detects type when not specified."""
        server = self._make_server()
        mock_storage = AsyncMock()
        mock_brain = MagicMock(id="test-brain", config=MagicMock())
        mock_storage.get_brain = AsyncMock(return_value=mock_brain)
...
class TestMCPMiscErrors:
    ...
    async def test_stats_no_brain(self, server: MCPServer) -> None:
        """Test smem_stats when no brain configured."""
        mock_storage = AsyncMock()
        mock_storage.get_brain = AsyncMock(return_value=None)
        mock_storage._current_brain_id = "test-brain"
        mock_storage.brain_id = "test-brain"
    ...
    async def test_session_unknown_action(self, server: MCPServer) -> None:
        """Test smem_session with unknown action."""
        mock_storage = AsyncMock()

        with patch.object(server, "get_storage", return_value=mock_storage):
            result = await server.call_tool("smem_session", {"action": "bogus"})

        assert "error" in result

    @pytest.mark.asyncio
    async def test_index_unknown_action(self, server: MCPServer) -> None:
        """Test smem_index with unknown action."""
        mock_storage = AsyncMock()

        with patch.object(server, "get_storage", return_value=mock_storage):
            result = await server.call_tool("smem_index", {"action": "bogus"})

        assert "error" in result

    @pytest.mark.asyncio
    async def test_index_scan_no_brain(self, server: MCPServer) -> None:
        """Test smem_index scan with no brain."""
        mock_storage = AsyncMock()
        mock_storage.get_brain = AsyncMock(return_value=None)
        mock_storage._current_brain_id = "test-brain"
        mock_storage.brain_id = "test-brain"
        ...
        with patch.object(server, "get_storage", return_value=mock_storage):
            result = await server.call_tool("smem_index", {"action": "scan"})

        assert "error" in result

    @pytest.mark.asyncio
    async def test_index_scan_not_directory(self, server: MCPServer) -> None:
        """Test smem_index scan with non-existent path."""
        mock_storage = AsyncMock()
        mock_brain = MagicMock(id="test-brain", config=MagicMock())
        mock_storage.get_brain = AsyncMock(return_value=mock_brain)
        mock_storage._current_brain_id = "test-brain"
    ...
    async def test_session_get_active(self, server: MCPServer) -> None:
        """Test smem_session get with active session returns data."""
        mock_storage = AsyncMock()
...
class TestMCPFireTrigger:
    ...
    def test_fire_trigger_increments_counter(self) -> None:
        ...
        with patch("surreal_memory.mcp.eternal_handler.check_triggers") as mock_check:
...
class TestMCPInputValidation:
    ...
    async def test_remember_rejects_oversized_content(self) -> None:
        ...
        with patch.object(server, "get_storage", return_value=mock_storage):
            result = await server.call_tool("smem_remember", {"content": "x" * 200_000})
    ...
    async def test_auto_analyze_rejects_oversized_text(self) -> None:
        ...
        result = await server.call_tool("smem_auto", {"action": "analyze", "text": "x" * 200_000})
    ...
    async def test_auto_process_rejects_oversized_text(self) -> None:
        ...
        result = await server.call_tool("smem_auto", {"action": "process", "text": "x" * 200_000})
    ...
    async def test_remember_rejects_invalid_type(self) -> None:
        ...
        with patch.object(server, "get_storage", return_value=mock_storage):
            result = await server.call_tool(
                "smem_remember", {"content": "test", "type": "invalid_type"}
            )
    ...
    async def test_recall_rejects_invalid_depth(self) -> None:
        ...
        with patch.object(server, "get_storage", return_value=mock_storage):
            result = await server.call_tool("smem_recall", {"query": "test", "depth": 99})
    ...
    async def test_suggest_rejects_invalid_type_filter(self) -> None:
        ...
        with patch.object(server, "get_storage", return_value=mock_storage):
            result = await server.call_tool(
                "smem_suggest", {"prefix": "test", "type_filter": "bogus"}
            )
