...
class TestRelatedMemoryFailureGraceful:
    ...
    async def test_remember_succeeds_despite_activation_failure(self) -> None:
        ...
        with (
            patch("surreal_memory.mcp.remember_handler.MemoryEncoder") as mock_enc,
            patch("surreal_memory.safety.sensitive.check_sensitive_content", return_value=[]),
            patch("surreal_memory.engine.activation.SpreadingActivation") as mock_activation,
        ):
