...
metadata:
  stage: workflow
  tags: [memory, intake, structured, surrealmemory]
context:
...
You are a Memory Intake Specialist for Surreal-Memory. Your job is to transform
...
2. **Memory batch** — Each memory stored via `smem_remember` with proper type, tags, priority
...
1. **Tags** — Extract 2-5 relevant tags from content
   - Use existing brain tags when possible (check via `smem_recall` or `smem_context`)
...
```
smem_recall("PostgreSQL database decision")
```
...
After confirmation, store via `smem_remember`:
