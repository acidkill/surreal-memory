...
**Current state**: v2.0.0 — 58 MCP tools, 6900+ tests, schema v40, SurrealDB backend (SQLite/InMemory retained only as test fixtures), neuroscience engine (10 brain-inspired algorithms), tiered memory loading (HOT/WARM/COLD).
...
- [x] COLD tier: explicit `smem_recall` only, 2× decay rate, excluded from auto-context
- [x] `smem_remember(..., tier="hot")` + `smem_edit` + `smem_recall(tier=...)` filter
- [x] `smem_pin` → auto-promote to HOT
...
**Vision**: Hybrid storage — hot data in memory + SurrealDB, warm in SQLite WAL, cold in compressed archives.
