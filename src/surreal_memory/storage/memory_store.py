"""In-memory storage backend using NetworkX — TEST FIXTURE ONLY.

v2.0.0 made SurrealDB the only production backend. ``InMemoryStorage``
stays in-tree to back unit-test fixtures that need a fast, in-process
graph without external services. Do not use it in production —
SurrealDB is the supported backend (see ``docker-compose.surrealdb.yml``).
"""
