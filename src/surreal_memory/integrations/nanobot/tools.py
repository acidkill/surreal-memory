"""Surreal-Memory tools for Nanobot's ToolRegistry.

Four tools conforming to Nanobot's Tool ABC interface:
- smem_remember — store a memory
- smem_recall — query with spreading activation
- smem_context — get recent context
- smem_health — brain diagnostics
"""
...
from surreal_memory.integrations.nanobot.base_tool import BaseNMTool


class NMRememberTool(BaseNMTool):
    """Store a memory in Surreal-Memory's neural graph."""

    @property
    def name(self) -> str:
        return "smem_remember"

    @property
    def description(self) -> str:
        return (
            "Store a memory in Surreal-Memory. Use this to remember facts, "
...
class NMRecallTool(BaseNMTool):
    ...
    def description(self) -> str:
        return (
            "Query memories from Surreal-Memory using spreading activation. "
...
class NMContextTool(BaseNMTool):
    ...
    def description(self) -> str:
        return (
            "Get recent context from Surreal-Memory. Use at the start of "
