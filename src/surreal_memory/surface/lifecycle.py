...
from surreal_memory.surface.resolver import save_surface_text
from surreal_memory.surface.serializer import serialize
from surreal_memory.surface.token_budget import trim_to_budget

if TYPE_CHECKING:
    from surreal_memory.surface.models import KnowledgeSurface
...
async def show_surface(brain_name: str = "default", *, global_only: bool = False) -> dict[str, Any]:
    ...
    if not text:
        return {
            ...
            "hint": "Run smem_surface(action='generate') to create one",
        }
