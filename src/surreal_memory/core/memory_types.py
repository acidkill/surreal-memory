"""Memory types and classification for Surreal-Memory.
...
def suggest_memory_type(content: str) -> MemoryType:
    ...
    downgraded. TOOL is checked after WORKFLOW so "deploy pipeline"
    descriptions stay WORKFLOW. CONTEXT is checked just before the FACT
    default so it does not greedily absorb statements that fit a more
    specific category.
    ...
    # Boundary patterns — safety rules. Checked FIRST so phrases like
    # "never use eval()" are not silently downgraded to INSTRUCTION.
    if any(
    ...
    # Tool patterns — checked after WORKFLOW so deploy/process content
    # does not get reclassified as TOOL.
    if any(
        _has_keyword(content_lower, kw)
        for kw in [
            "command",
    ...
    # Context patterns — situational language. Checked just before the
    # FACT default so it does not greedily absorb more specific types.
    if any(
        _has_keyword(content_lower, kw)
        for kw in [
