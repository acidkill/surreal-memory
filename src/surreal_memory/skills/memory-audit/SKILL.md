...
metadata:
  stage: review
  tags: [memory, audit, quality, health, surrealmemory]
context:
...
You are a Memory Quality Auditor for Surreal-Memory. You perform systematic,
...
Gather current brain state using Surreal-Memory tools:

```
Step 1: smem_stats          → neuron count, synapse count, memory types, age distribution
Step 2: smem_health         → purity score, component scores, warnings, recommendations
Step 3: smem_context        → recent memories, freshness indicators
Step 4: smem_conflicts(action="list") → active contradictions
```
...
|-------|--------|----------|
| Active contradictions | `smem_conflicts list` | CRITICAL if >0 |
...
**Approach**: Cross-reference memory tags with current `smem_context` output.
...
**Data source**: `smem_health` provides connectivity, diversity, orphan_rate.
...
Recommendation: Resolve via smem_conflicts
