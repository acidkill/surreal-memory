...
Periodically checks PyPI for newer versions of surreal-memory and caches
...
_PYPI_URL = "https://pypi.org/pypi/surreal-memory/json"
...
class VersionCheckHandler:
    ...
    async def _check_latest_version(self) -> None:
        ...
        try:
            ...
            if update_available:
                logger.info(
                    "Surreal-Memory update available: %s -> %s",
                    __version__,
                    latest,
                )
        except Exception:
    ...
    def get_update_hint(self) -> dict[str, Any] | None:
        ...
        return {
            "message": (
                f"Surreal-Memory v{self._version_info.latest} is available "
                f"(current: v{self._version_info.current}). "
                f"Run: pip install --upgrade surreal-memory"
            ),
            "current_version": self._version_info.current,
            "latest_version": self._version_info.latest,
        }
