...
class SessionHandler:
    ...
    async def _session_get(self, storage: NeuralStorage) -> dict[str, Any]:
        ...
        the agent can trigger ``smem_auto(action="flush")``.
        """
        ...
        if not session or not session.metadata.get("active", True):
            ...
            if gap:
                ...
                result["gap_message"] = (
                    ...
                    "Consider running smem_auto(action='flush') with recent conversation."
                )
            return result
    ...
    # ── SITUATION (agent-ergonomics) ──

    async def _resolve_content(self, tm: TypedMemory, storage: NeuralStorage) -> str:
        """Resolve display content for a TypedMemory.

        Tries metadata['content'] first (cheap), then fiber summary, then the
        anchor neuron's content. Returns empty string on any failure.
        """
    ...
    async def _situation(self, _args: dict[str, Any]) -> dict[str, Any]:
        ...
        Aggregates active session state, top recent decisions, and open blockers
        in a single response so agents can resume context without chaining
        ``smem_recap`` + multiple ``smem_recall`` calls.
        ...
        # files_in_session is contract-stable empty until session metadata tracks
        # edited files (out of scope here). Keep the key so consumers can rely on it.
        result: dict[str, Any] = {
        ...
        if gap_detected:
            result["suggestion"] = (
                "Gap detected — run smem_auto(action='flush') with recent conversation."
            )
        elif active_meta is None and not recent_decisions:
            result["suggestion"] = (
                "Fresh brain — call smem_session(action='set', feature='...', task='...') "
                "to anchor your work."
            )
        elif open_blockers:
            result["suggestion"] = (
