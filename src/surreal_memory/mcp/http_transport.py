"""Streamable HTTP transport for the Surreal-Memory MCP server.
...
Usage:
    smem-mcp --http                    # default port 8765
    smem-mcp --http 9000              # custom port
...
Note: All clients share a single MCPServer instance. This means:
    - Session state (smem_session) is shared — last writer wins
...
async def run_http_server(port: int = 8765, host: str = "127.0.0.1") -> None:
    ...
    from surreal_memory.mcp.server import (
        _lazy_init,
        create_mcp_server,
        handle_message,
    )
    ...
    logger.info("Surreal-Memory MCP HTTP transport on http://%s:%d/mcp", host, port)
    print(  # noqa: T201
        f"Surreal-Memory MCP server (HTTP) listening on http://{host}:{port}/mcp",
        flush=True,
    )
