...
from surreal_memory.engine.stabilization import StabilizationConfig, stabilize
...
if TYPE_CHECKING:
    ...
    from surreal_memory.engine.depth_prior import AdaptiveDepthSelector, DepthDecision
    from surreal_memory.engine.embedding.provider import EmbeddingProvider
    from surreal_memory.engine.ppr_activation import PPRActivation
...
class ReflexPipeline:
    ...
    def __init__(
        ...
        if config.adaptive_depth_enabled:
            from surreal_memory.engine.depth_prior import AdaptiveDepthSelector
    ...
    def _get_encryptor(self) -> Any:
        ...
        try:
            from surreal_memory.unified_config import get_config as _get_cfg
    ...
    async def query(
        ...
        if session_id and _session_state is not None:
            try:
                from surreal_memory.engine.priming import (
                    ActivationCache,
                    PrimingMetrics,
                    compute_priming,
        ...
        if strategy == "cone":
            # InfinityDB-backed cone (HNSW) was a Pro-plugin retrieval path; it
            # was retired in v2.0.0 with the InfinityDB chain. Fall back to
            # classic activation transparently.
            logger.debug("cone strategy retired — falling back to classic activation")
            strategy = "classic"
    ...
    async def _try_temporal_reasoning(
        ...
        if traversal == "temporal_range" and stimulus.time_hints:
            ...
            from surreal_memory.engine.causal_traversal import query_temporal_range
    ...
    async def _find_anchors_ranked(
        ...
        if self._config.idf_anchor_enabled:
            try:
                from surreal_memory.engine.idf_anchor import compute_keyword_limits
        ...
        if self._config.fuzzy_search_enabled and len(keyword_anchors) < 2:
            try:
                from surreal_memory.engine.fuzzy_match import (
                    find_fuzzy_matches,
    ...
    async def _find_matching_fibers(
        ...
        def _fiber_score(fiber: Fiber) -> float:
            ...
            if fw > 0.0 and fiber.created_at:
                from surreal_memory.safety.freshness import evaluate_freshness
            ...
            if getattr(self._config, "context_retrieval_enabled", True):
                ...
                if stored_fp and isinstance(stored_fp, dict) and query_tokens:
                    from surreal_memory.engine.context_retrieval import (
                        ContextFingerprint,
                        context_match_score,
                    )
