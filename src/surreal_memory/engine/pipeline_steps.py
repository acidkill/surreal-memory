...
from surreal_memory.extraction.sentiment import SentimentExtractor, Valence
...
class DecisionComponentStep:
    ...
    async def execute(
        ...
        from surreal_memory.engine.decision_intel import extract_decision_components
...
class ConflictDetectionStep:
    ...
    async def execute(
        ...
        from surreal_memory.engine.conflict_detection import (
            detect_conflicts,
            resolve_conflicts,
        )
        ...
        try:
            from surreal_memory.engine.conflict_auto_resolve import try_auto_resolve
...
class BuildFiberStep:
    ...
    async def execute(
        ...
        if ctx.merged_tags and len(ctx.merged_tags) >= 2:
            try:
                await storage.record_tag_cooccurrence(ctx.merged_tags)
            except Exception:
