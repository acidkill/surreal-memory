...
Usage standalone:
    echo '{"transcript_path": "/path/to/transcript.jsonl"}' | python -m surreal_memory.hooks.pre_compact
    python -m surreal_memory.hooks.pre_compact --transcript /path/to/transcript.jsonl
    python -m surreal_memory.hooks.pre_compact --text "Some text to flush"
"""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Surreal-Memory PreCompact hook — flush memories before compaction"
    )
    parser.add_argument(
        "--transcript",
        "-t",
    ...
    try:
        ...
        if saved > 0:
            print(  # noqa: T201
                f"[Surreal-Memory] Pre-compact flush: captured {saved} memories",
                file=sys.stderr,
            )
        else:
            print(  # noqa: T201
                f"[Surreal-Memory] Pre-compact flush: {result.get('message', 'no memories')}",
                file=sys.stderr,
            )
    except Exception:
        print("[Surreal-Memory] Pre-compact flush failed", file=sys.stderr)  # noqa: T201
