"""Interactive first-run wizard for Surreal-Memory.

Guides new users through brain setup, embedding provider selection,
and a test encode/recall cycle. Called by `smem init --wizard`.
"""
...
_PROVIDERS: list[dict[str, str]] = [
    ...
    },
    {
        ...
        "install": "pip install surreal-memory[embeddings-gemini]",
    ...
    },
    {
        "key": "openai",
        "name": "OpenAI (cloud)",
        "install": "pip install surreal-memory[embeddings-openai]",
        "note": "Paid, needs OPENAI_API_KEY",
    },
    {
        "key": "openrouter",
        "name": "OpenRouter (cloud)",
        "install": "pip install surreal-memory[embeddings-openrouter]",
...
def run_wizard(*, force: bool = False) -> None:
    ...
    typer.echo()
    typer.secho("  Surreal-Memory Setup Wizard", bold=True)
    ...
    if provider:
        ...
        results["Embedding"] = "skipped (can set up later with: smem setup embeddings)"
...
def _step_test_memory(brain_name: str) -> None:
    ...
    content = typer.prompt(
        "  Memory content",
        default="Surreal-Memory was set up successfully!",
    ).strip()
