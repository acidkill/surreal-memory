...
Checks PyPI for newer versions of surreal-memory, cached for 24 hours.
Runs in background thread to avoid slowing down CLI commands.

Cache location: ~/.surrealmemory/.update_check
"""
...
PYPI_URL = "https://pypi.org/pypi/surreal-memory/json"
...
def _get_cache_path() -> Path:
    ...
    data_dir = os.environ.get("SURREAL_MEMORY_DIR") or str(Path.home() / ".surrealmemory")
...
def _fetch_latest_version() -> str | None:
    ...
    try:
        ...
        req = Request(
            PYPI_URL,
            headers={"Accept": "application/json", "User-Agent": "surreal-memory-cli"},
        )
...
def _is_editable_install() -> bool:
    """Detect if surreal-memory is installed in editable/development mode."""
    try:
        from importlib.metadata import distribution

        dist = distribution("surreal-memory")
    ...
    try:
        ...
        pkg_dir = Path(surreal_memory.__file__).resolve().parent
...
def _print_update_notice(current: str, latest: str) -> None:
    ...
    if _is_editable_install():
        ...
        notice = (
            f"\n  Update available: surreal-memory {current} → {latest}\n"
            f"  Run: pip install -U surreal-memory\n"
        )
    # Use stderr so it doesn't break piped output (e.g. smem recall ... | jq)
