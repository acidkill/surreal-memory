...
from surreal_memory.safety.freshness import analyze_freshness, format_age
    ...
    if json_output:
        ...
        if result.get("typed_memory_count", 0) > 0:
            ...
            if result.get("expired_count", 0) > 0:
                typer.secho(
                    f"\n  [!] {result['expired_count']} expired memories - run 'smem cleanup' to remove",
...
def check(
    ...
    Examples:
        smem check "My API_KEY=sk-xxx123"
        smem check "password: secret123" --json
    """
...
def status(
    ...
    async def _status() -> dict[str, Any]:
        ...
        if len(expired) > 0:
            suggestions.append(f"{len(expired)} expired memories — run 'smem cleanup' to remove")

        if len(todos) > 0:
            suggestions.append(f"{len(todos)} pending TODOs")

        if stats_data["fiber_count"] == 0:
            suggestions.append('No memories yet — try: smem remember "your first memory"')
