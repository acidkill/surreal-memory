...
def brain_list(
    ...
    if json_output:
        ...
        if not brains:
            typer.echo("No brains found. Create one with: smem brain create <name>")
            return
...
def brain_use(
    ...
    This updates config.toml so CLI and MCP servers (without SURREAL_MEMORY_BRAIN
    env var) will use the new brain. MCP servers started with SURREAL_MEMORY_BRAIN
    are pinned to that brain and will NOT be affected by this command.

    Examples:
        smem brain use work
        smem brain use personal
    """
    import os
...
def brain_create(
    ...
    Examples:
        ...
        smem brain create personal --no-use
    """
...
def brain_export(
    ...
    Examples:
        ...
        smem brain export --format markdown -o brain.md
    ...
    async def _export() -> None:
        ...
        # Format output
        if fmt == "markdown":
            from surreal_memory.cli.markdown_export import snapshot_to_markdown
...
def brain_import(
    ...
    Examples:
        ...
        smem brain import shared-brain.json --name shared
...
def brain_delete(
    ...
    Examples:
        smem brain delete old-brain
        smem brain delete temp --force
    """
    config = get_config()
...
def _display_health(result: dict[str, Any]) -> None:
    ...
    if result["sensitive_content"]["count"] > 0:
        ...
        typer.secho(
            "  Run 'smem brain export --exclude-sensitive' for safe export",
...
def brain_transplant(
    ...
    Examples:
        smem brain transplant expert-brain --tag python --tag api
        smem brain transplant work-brain --type fact --type decision
        smem brain transplant shared --strategy prefer_recent
    """

    async def _transplant() -> dict[str, Any]:
        from surreal_memory.engine.brain_transplant import TransplantFilter, transplant
        from surreal_memory.engine.merge import ConflictStrategy
...
def brain_health(
    ...
    Examples:
        smem brain health
        smem brain health --name work --json
    """
