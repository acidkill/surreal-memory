...
def watch(
    ...
    Examples:
        smem watch ~/notes              # One-shot scan
        smem watch ~/notes -a start     # Start background watching
        smem watch -a stop              # Stop watching
        smem watch -a status            # Show watcher status
    """
    ...
    async def _watch() -> dict[str, Any]:
        ...
        if action == "stop":
            return {
                "action": "stop",
                "message": "Use MCP smem_watch(action='stop') for background watcher",
            }
