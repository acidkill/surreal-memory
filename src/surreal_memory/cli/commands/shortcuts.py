...
def quick_add(
    ...
    Examples:
        smem a "API key format is sk-xxx"
...
def show_last(
    ...
    Examples:
        smem last           # Show last 5 memories
        smem last -n 10     # Show last 10 memories
    """
...
def mcp_config(
    ...
    Examples:
        smem mcp-config                    # Basic config
        smem mcp-config --with-prompt      # Include system prompt
        smem mcp-config -p -c              # Include compact prompt
    """
    import shutil
    import sys

    from surreal_memory.mcp.prompt import get_system_prompt

    # Find smem executable path
    smem_path = shutil.which("smem") or shutil.which("smem-mcp") or sys.executable

    config = {
        "surreal-memory": {
            "command": smem_path if "python" not in smem_path.lower() else "python",
            "args": ["-m", "surreal_memory.mcp"] if "python" in smem_path.lower() else ["mcp"],
        }
    }

    # Simplify if smem is available
    if shutil.which("smem-mcp"):
        config["surreal-memory"] = {"command": "smem-mcp", "args": []}
...
def prompt(
    ...
    to use Surreal-Memory for persistent memory across sessions.

    Examples:
        smem prompt              # Show full prompt
        smem prompt --compact    # Show shorter version
        smem prompt --copy       # Copy to clipboard
    """
    from surreal_memory.mcp.prompt import get_system_prompt
...
def export_brain_cmd(
    ...
    Examples:
        smem export backup.json           # Export current brain
        smem export work.json -b work     # Export specific brain
    """
...
def import_brain_cmd(
    ...
    Examples:
        smem import backup.json                          # Import (replace)
        smem import backup.json -b new                   # Import as 'new' brain
        smem import backup.json --merge                  # Merge into existing brain
        smem import backup.json --merge --strategy prefer_recent
    """
    from pathlib import Path
    ...
    async def _import() -> None:
        ...
        if merge:
            ...
            from surreal_memory.engine.merge import ConflictStrategy, merge_snapshots
