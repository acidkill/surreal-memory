"""Extended init — smem init --full.

Orchestrates a complete Surreal-Memory setup in one command:
...
_PROVIDER_PRIORITY: list[dict[str, str]] = [
    {
        ...
        "install": "pip install surreal-memory[embeddings]",
    },
    {
        "key": "gemini",
        "module": "google.genai",
    ...
    if choice == "1":
        ...
        try:
            result = subprocess.run(
                [sys.executable, "-m", "pip", "install", "surreal-memory[embeddings]"],
...
_BASH_MAINTENANCE = """\
#!/bin/bash
# Surreal-Memory maintenance script — generated by smem init --full
# Add to crontab: crontab -e → 0 3 * * * {script_path}
set -e

echo "[$(date)] Surreal-Memory maintenance starting..."

# 1. Memory decay (forgetting curve)
smem decay

# 2. Consolidate (prune + mature + dedup)
smem consolidate --strategy all

# 3. Health check
smem doctor

echo "[$(date)] Surreal-Memory maintenance complete."
"""
...
Write-Host "[$(Get-Date)] Surreal-Memory maintenance starting..."
...
Write-Host "[$(Get-Date)] Surreal-Memory maintenance complete."
"""
...
def print_full_banner(results: dict[str, str]) -> None:
    ...
    typer.secho("  |                                                  |", dim=True)
    typer.echo(
        "  |  " + typer.style("Surreal-Memory is ready!", bold=True) + "                   |"
    )
    typer.secho("  |                                                  |", dim=True)
    ...
    typer.secho("  |                                                  |", dim=True)
    typer.echo(
        "  |  Run " + typer.style("smem doctor", bold=True) + " to verify your setup        |"
    )
    ...
    if skip_embeddings:
        ...
        if provider is None:
            ...
            if sys.stdin.isatty():
                ...
                typer.echo("  Run interactively or use: pip install surreal-memory[embeddings]")
