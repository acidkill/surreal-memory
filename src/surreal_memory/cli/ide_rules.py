...
Gemini, etc.) to use Surreal-Memory effectively.

Usage:
    smem setup rules              # Interactive: choose IDEs
    smem setup rules --all        # Generate for all supported IDEs
    smem setup rules --ide cursor # Single IDE
"""
...
```
smem_recall("current project context")
smem_recall("<specific topic you're working on>")
```
...
| Action | When | Command |
|--------|------|---------|
| Check brain health | Weekly | `smem_health()` |
| Consolidate memories | Weekly | `smem consolidate` |
| Review queue | Monthly | `smem_review(action="queue")` |
...
def run_ide_rules_setup(
    *,
    ...
    typer.echo()
    typer.echo("  Rules tell your IDE agent to use Surreal-Memory automatically.")
