...
- **Extension ID typo** in update checker — was `surrealmemory.surrealmemory`, corrected to `neuralmem.surrealmemory`. Prevents false "update available" notifications.
- **autoStart default mismatch** — config.ts fallback was `false` while package.json declared `true`. Now both use `true`.
- **Brain switch not persisted** on fresh install — `writeCurrentBrain()` now creates `~/.surrealmemory/config.toml` if it doesn't exist, instead of silently returning.
...
- Synced with Surreal-Memory v2.10.2
...
- Synced with Surreal-Memory v0.14.0: relation extraction engine
...
- Synced with Surreal-Memory v0.13.0: cognitive runtime upgrade
...
- Synced with Surreal-Memory v0.9.5: OpenMemory-inspired intelligence features
...
- Synced with Surreal-Memory v0.9.4: retrieval pipeline 3-5x faster
...
- Synced with Surreal-Memory v0.9.3
...
- Synced with Surreal-Memory v0.9.2 security hardening
...
- Synced with Surreal-Memory v0.9.2: 3-tier auto-save, trigger engine, lazy loading
...
- Sync with Surreal-Memory v0.9.1 system optimizations
