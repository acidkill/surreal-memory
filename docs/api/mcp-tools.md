...
- [Core Memory](#core)
  - [`smem_remember`](#smem_remember)
  - [`smem_remember_batch`](#smem_remember_batch)
  - [`smem_recall`](#smem_recall)
  - [`smem_show`](#smem_show)
  - [`smem_context`](#smem_context)
  - [`smem_todo`](#smem_todo)
  - [`smem_auto`](#smem_auto)
  - [`smem_suggest`](#smem_suggest)
- [Session & Context](#session)
  - [`smem_session`](#smem_session)
  - [`smem_eternal`](#smem_eternal)
  - [`smem_recap`](#smem_recap)
- [Provenance & Sources](#provenance)
  - [`smem_provenance`](#smem_provenance)
  - [`smem_source`](#smem_source)
- [Analytics & Health](#analytics)
  - [`smem_stats`](#smem_stats)
  - [`smem_health`](#smem_health)
  - [`smem_evolution`](#smem_evolution)
  - [`smem_habits`](#smem_habits)
  - [`smem_narrative`](#smem_narrative)
- [Cognitive Reasoning](#cognitive)
  - [`smem_hypothesize`](#smem_hypothesize)
  - [`smem_evidence`](#smem_evidence)
  - [`smem_predict`](#smem_predict)
  - [`smem_verify`](#smem_verify)
  - [`smem_cognitive`](#smem_cognitive)
  - [`smem_gaps`](#smem_gaps)
  - [`smem_schema`](#smem_schema)
  - [`smem_explain`](#smem_explain)
- [Training & Import](#training)
  - [`smem_train`](#smem_train)
  - [`smem_train_db`](#smem_train_db)
  - [`smem_index`](#smem_index)
  - [`smem_import`](#smem_import)
- [Memory Management](#management)
  - [`smem_edit`](#smem_edit)
  - [`smem_forget`](#smem_forget)
  - [`smem_pin`](#smem_pin)
  - [`smem_consolidate`](#smem_consolidate)
  - [`smem_review`](#smem_review)
  - [`smem_alerts`](#smem_alerts)
- [Cloud Sync & Backup](#sync)
  - [`smem_sync`](#smem_sync)
  - [`smem_sync_status`](#smem_sync_status)
  - [`smem_sync_config`](#smem_sync_config)
  - [`smem_telegram_backup`](#smem_telegram_backup)
- [Versioning & Transfer](#meta)
  - [`smem_version`](#smem_version)
  - [`smem_transplant`](#smem_transplant)
  - [`smem_conflicts`](#smem_conflicts)
- [Other](#other)
  - [`smem_reasoning`](#smem_reasoning)
  - [`smem_uncertainty`](#smem_uncertainty)
  - [`smem_visualize`](#smem_visualize)
  - [`smem_watch`](#smem_watch)
  - [`smem_surface`](#smem_surface)
  - [`smem_tool_stats`](#smem_tool_stats)
  - [`smem_lifecycle`](#smem_lifecycle)
  - [`smem_refine`](#smem_refine)
...
| `type` | string (`fact`, `decision`, `preference`, `todo`, `insight`, `context`, `instruction`, `error`, `workflow`, `reference`, `boundary`) | No | — | Memory type (auto-detected if not specified). Some types default to a non-null expiry when expires_days is omitted: t... |
...
| `expires_days` | integer | No | — | Days until memory expires. If omitted, falls back to the type's default (todo=30d, insight=180d, workflow=365d; decis... |
...
Store multiple memories in a single call. Max 20 items, 500K total chars. Each item supports the same fields as smem_remember. Returns per-item results (partial success — one bad item won't block the rest).
...
Create, list, or inspect hypotheses — evolving beliefs with Bayesian confidence tracking. Hypotheses auto-resolve when evidence is strong enough (confirmed at >=0.9 confidence with >=3 evidence-for, refuted at <=0.1 with >=3 evidence-against). Use smem_evidence to add supporting/opposing evidence.
...
Create, list, or inspect predictions — falsifiable claims about future observations. Predictions track confidence, optional deadlines, and can link to hypotheses via PREDICTED synapse. Verified predictions propagate evidence back to linked hypotheses. Use smem_verify to record outcomes.
...
Train brain from documentation files. Supports PDF, DOCX, PPTX, HTML, JSON, XLSX, CSV (requires: pip install surreal-memory[extract]). Trained memories are pinned by default (no decay, no compression, permanent KB).
...
| `extensions` | array[string] | No | default: ['.md'] | File extensions to include (default: ['.md']). Rich formats (PDF, DOCX, PPTX, HTML, XLSX) require: pip install surrea... |
...
Reasoning training: status, mine, patterns, or config.

| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| `action` | string (`status`, `mine`, `patterns`, `config`) | Yes | — | status=trace/pattern stats + per-model coverage, mine=scan ~/.claude thinking and distill patterns (needs mining_enab... |
| `model` | string | No | — | Filter patterns by source model (patterns action) |
| `category` | string | No | — | Filter patterns by category (patterns action) |
...
| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| `neuron_id` | string | Yes | — | ID of the instruction or workflow memory to refine (use the fiber_id returned by smem_remember or smem_recall). |
