"""Surreal-Memory demo script -- run this to generate a terminal recording.
...
def main() -> None:
    """Run the Surreal-Memory demo."""
    print()
    print(f"{BOLD}  Surreal-Memory Demo{RESET}")
    print(f"{DIM}  Reflex-based memory for AI agents{RESET}")
    pause(1.5)

    # ── 1. Install ──
    section("1. Install")
    prompt("pip install surreal-memory", delay=0.02)
    output(f"{DIM}Successfully installed surreal-memory-1.6.0{RESET}")
    pause(1)

    # ── 2. Store memories ──
    section("2. Store Memories")

    prompt('smem remember "Fixed auth bug with null check in login.py:42"')
    pause(0.3)
    output(f"  {GREEN}Encoded{RESET} | neurons: 4 | synapses: 3 | fiber: f-a1b2c3")
    pause(0.8)

    prompt('smem remember "We decided to use PostgreSQL over MongoDB" --type decision')
    pause(0.3)
    output(f"  {GREEN}Encoded{RESET} | neurons: 3 | synapses: 2 | fiber: f-d4e5f6")
    pause(0.8)

    prompt('smem remember "Alice suggested JWT for auth, caused Tuesday outage"')
    pause(0.3)
    output(f"  {GREEN}Encoded{RESET} | neurons: 5 | synapses: 6 | fiber: f-g7h8i9")
    pause(0.8)

    prompt('smem todo "Review PR #123" --priority 7')
    pause(0.3)
    ...
    prompt('smem recall "auth bug fix"')
    pause(0.5)
    output(f"""
...
""")
    ...
    prompt('smem recall "why did the outage happen" --depth 3')
    pause(0.5)
    output(f"""
...
""")
    ...
    prompt('smem recall "how are orders and customers related"')
    pause(0.5)
    output(f"""
  {BOLD}Result:{RESET}
...
""")
    ...
    print(f"{GREEN}{BOLD}  That's Surreal-Memory.{RESET}")
    print(f"{DIM}  pip install surreal-memory{RESET}")
    print(f"{DIM}  github.com/acidkill/surreal-memory{RESET}")
    print(f"{GREEN}{BOLD}{'-' * 50}{RESET}")
    print()
