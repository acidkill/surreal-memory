...
`smem graph` outputs Rich Tree to terminal only. Users cannot:
...
- Surreal-Memory graph data is simple (tree of fibers → neurons → synapses)
...
- Overkill for the tree-like structure Surreal-Memory produces
...
### In Scope
- `smem graph --export svg` → writes `.svg` file
- `smem graph --export png` → writes `.png` file (via `cairosvg` optional dep)
...
```
smem graph                           # Current behavior (Rich Tree in terminal)
smem graph --export svg              # Export to SVG file
smem graph --export png              # Export to PNG file (requires cairosvg)
smem graph "query" --export svg      # Query-filtered graph → SVG
smem graph --export svg -o out.svg   # Custom output path
```
...
def svg_to_png(svg_content: str, output_path: Path) -> None:
    try:
        ...
        raise click.UsageError(
            "PNG export requires cairosvg: pip install surreal-memory[png]"
        )
```
