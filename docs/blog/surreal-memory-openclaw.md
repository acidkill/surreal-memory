# OpenClaw Mất Trí Nhớ Sau 45 Giờ — Surreal-Memory Giải Quyết Thế Nào
...
## Giải pháp: Surreal-Memory — Bộ Não Cho Đôi Tay OpenClaw
...
[Surreal-Memory](https://github.com/acidkill/surreal-memory) (v1.3.0) là hệ thống bộ nhớ dựa trên **neural graph** — không dùng embedding, không dùng vector search, không cần LLM. Thay vào đó, nó mô phỏng cách não người hoạt động:
...
Surreal-Memory: SQLite graph (không giới hạn) → recall bất kỳ lúc nào → KHÔNG BAO GIỜ MẤT
```

Mọi memory được lưu vào SQLite neural graph tại `~/.surrealmemory/brains/`. Agent có thể recall thông tin từ tuần 1 khi đang ở tuần 12. Không phụ thuộc context window.
...
OpenClaw MEMORY.md giữ mọi thứ mãi mãi. Surreal-Memory tự quên cái tạm thời, giữ cái quan trọng — **giống cách não người hoạt động**.

#### 3. Tự phát hiện mâu thuẫn (không cần LLM)

```python
# Tuần 1
smem_remember("Chúng ta dùng PostgreSQL cho database", type="decision")

# Tuần 4
smem_remember("Chuyển sang MySQL cho database", type="decision")

# Surreal-Memory tự động:
...
→ Surreal-Memory tự tạo CONCEPT neuron:
...
smem version save --name "pre-refactor-v2"

# Nếu hỏng, rollback
smem version rollback --to "pre-refactor-v2"

# Transplant Python knowledge từ expert brain
smem transplant --from expert.db --tags python,patterns --to my-brain
```
...
→ Surreal-Memory phát hiện pattern:
...
| | OpenClaw Memory | Surreal-Memory |
|-|----------------|---------------|
...
Surreal-Memory là **pure algorithmic** — 1,352 tests, Python thuần, SQLite storage, 16 MCP tools. Không phụ thuộc bất kỳ AI service nào.
...
Tạo file `.openclaw/skills/surreal-memory/SKILL.md`:
...
# Surreal-Memory Skill

Persistent memory system using neural graph + spreading activation.
16 MCP tools: smem_remember, smem_recall, smem_context, smem_todo,
smem_auto, smem_temporal, smem_habits, smem_stats, smem_health,
smem_version, smem_transplant, smem_consolidate, smem_update, ...
```
...
| Tiêu chí | OpenClaw Memory | Mem0 Plugin | Surreal-Memory |
|----------|----------------|-------------|---------------|
...
## Real-world: 12 tuần với OpenClaw + Surreal-Memory
...
Agent: smem_remember("Tech stack: Next.js 14 + Prisma + PostgreSQL",
                      type="decision", priority=8)
Agent: smem_remember("Alice là tech lead backend", type="fact", priority=6)
Agent: smem_index(action="scan", path="./src")
...
Agent: smem_remember("Auth crash: null token in JWT verify", type="error", priority=7)
Agent: smem_remember("Fix: add null check before jwt.verify()", type="error", priority=7)
```

**OpenClaw compaction xảy ra ở đây.** Không sao — Surreal-Memory giữ tất cả.

### Tuần 6: Quyết định thay đổi

```
User: "Chuyển từ REST sang GraphQL"
Agent: smem_remember("Switch REST → GraphQL for API", type="decision", priority=8)

→ Surreal-Memory auto-detect: không mâu thuẫn (REST chưa được ghi là decision)
...
Agent: smem_remember("Switch to MySQL", type="decision", priority=8)

→ Surreal-Memory auto-detect:
...
```
Agent: smem_recall("ai thường handle database issues?")
...
**OpenClaw + Surreal-Memory ở tuần 12**: 100% context intact. Chuỗi nhân quả PostgreSQL → MySQL rõ ràng. Pattern "Alice = database person" tự hình thành. Zero information loss.

---

## Benchmark: Surreal-Memory vs Keyword Baseline

Surreal-Memory v1.0 đi kèm evaluation framework với 30 ground-truth memories và 25 queries:

| Query category | Keyword search | Surreal-Memory | Improvement |
|---------------|---------------|---------------|-------------|
...
*[Surreal-Memory](https://github.com/acidkill/surreal-memory) — Reflex-based memory system for AI agents. Retrieval through activation, not search. v1.3.0, 1,352 tests, 16 MCP tools.*

*[OpenClaw Plugin](https://www.npmjs.com/package/surrealmemory) — Native OpenClaw integration. `npm install -g surrealmemory`*

*[ClawHub Skill](https://clawhub.ai/skills/surreal-memory) — Install Surreal-Memory as an OpenClaw skill in one click.*

*[Dashboard](http://localhost:8000/dashboard) — Neural graph explorer, integration status, health diagnostics. `smem serve`*
