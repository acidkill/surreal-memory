...
## Building a Surreal-Memory System

Surreal-Memory implements this model directly:
...
When you tell Surreal-Memory "Alice suggested we use PostgreSQL because MongoDB failed the benchmark on Tuesday", it:
...
When you ask "why did the Tuesday outage happen?", Surreal-Memory:
...
Human memory consolidates during sleep — strengthening important connections, pruning weak ones, cross-linking related memories. Surreal-Memory has a consolidation engine that does the same:
...
The key insight: **for simple lookups, they're equivalent. For reasoning, Surreal-Memory wins.** And AI agents increasingly need reasoning, not just lookup.
...
Everything is async Python. Storage is pluggable (SQLite default, in-memory for testing). The MCP server exposes 58 tools that any AI assistant can call.

### Numbers

- **1,649 tests** passing
- **58 MCP tools** (remember, recall, context, train, conflicts, health, habits, ...)
...
I use Surreal-Memory daily with Claude Code. My agent remembers:
...
Surreal-Memory is open source (MIT). Current focus areas:
...
**GitHub:** [github.com/acidkill/surreal-memory](https://github.com/acidkill/surreal-memory)
