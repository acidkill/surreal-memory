...
- [Other](#other)
  - [`smem brain recall-check`](#smem-brain-recall-check)
  - [`smem lifecycle`](#smem-lifecycle)
  - [`smem prune-orphan-states`](#smem-prune-orphan-states)
  ...
  - [`smem recover-semantic-discovery`](#smem-recover-semantic-discovery)
  - [`smem reindex`](#smem-reindex)
  - [`smem shared activate`](#smem-shared-activate)
  - [`smem storage status`](#smem-storage-status)
  - [`smem surface generate`](#smem-surface-generate)
  - [`smem surface show`](#smem-surface-show)
...
### `smem brain recall-check`

Probe whether sampled stored memories can be recalled through the normal retrieval path.

```
smem brain recall-check [OPTIONS]
```

| Option | Type | Required | Default | Description |
|--------|------|----------|---------|-------------|
| `--name / -n` | text | No | — | Brain name (default: current) |
| `--sample-size` | integer range | No | `10` | Number of stored memories to probe |
| `--json / -j` | boolean | No | `False` | Output as JSON |
...
### `smem prune-orphan-states`

Report — and optionally delete — neuron_state rows whose neuron is gone.

```
smem prune-orphan-states [OPTIONS]
```

| Option | Type | Required | Default | Description |
|--------|------|----------|---------|-------------|
| `--brain / -b` | text | No | — | Brain to inspect |
| `--apply` | boolean | No | `False` | Actually delete (default is a dry run) |
| `--limit / -l` | integer | No | `50000` | Max rows to examine in one pass |
...
| Option | Type | Required | Default | Description |
|--------|------|----------|---------|-------------|
| `--run-id` | text | Yes | — | Exact unfinished run ID |
...
```
smem surface generate [OPTIONS]
```

| Option | Type | Required | Default | Description |
|--------|------|----------|---------|-------------|
| `--brain / -b` | text | No | — | Brain name |
| `--global-path` | boolean | No | `False` | Write to ~/.surrealmemory/surfaces/<brain>.nm regardless of the directory |
| `--token-budget` | integer | No | `1200` | Max tokens for the surface |
| `--max-nodes` | integer | No | `30` | Max graph nodes to include |
| `--json / -j` | boolean | No | `False` | Output as JSON |
...
```
smem surface show [OPTIONS]
```

| Option | Type | Required | Default | Description |
|--------|------|----------|---------|-------------|
| `--brain / -b` | text | No | — | Brain name |
| `--global-path` | boolean | No | `False` | Read the global surface instead of a project one |
| `--raw` | boolean | No | `False` | Print the raw .nm text |
| `--json / -j` | boolean | No | `False` | Output as JSON |
...
*Auto-generated by `scripts/gen_cli_docs.py` from Typer app introspection — 85 commands.*
