...
!!! info "Test Setup"
    - **Surreal-Memory**: SQLiteStorage, spreading activation, zero external APIs
...
| Operation | Surreal-Memory | Mem0 | Speedup |
|-----------|:------------:|:----:|:-------:|
...
Mem0's write bottleneck: every `add()` call triggers an LLM call to extract and summarize the memory. Surreal-Memory encodes directly into neural structures — no LLM needed.

### Accuracy

| Metric | Surreal-Memory | Mem0 | Winner |
|--------|:------------:|:----:|:------:|
| **Semantic accuracy** (Jaccard) | 0.141 | 0.141 | Tie |
| **Multi-hop reasoning** (keyword coverage) | **0.417** | 0.383 | Surreal-Memory |
| **Conversation context** | 0.174 | 0.174 | Tie |

Equal accuracy on direct recall, but Surreal-Memory outperforms on multi-hop queries thanks to graph-based spreading activation — connections between memories are first-class citizens, not afterthoughts.

### Cost

| Metric | Surreal-Memory | Mem0 |
|--------|:------------:|:----:|
...
At scale, the cost difference is decisive. Mem0 requires an LLM call for every operation — Surreal-Memory's retrieval is purely algorithmic.

### Verdict

```
Surreal-Memory wins : 4 / 6 categories
Mem0 wins         : 0 / 6 categories
Ties              : 2 / 6 categories
```

**Surreal-Memory is 121x faster on writes, equally accurate, and costs $0 in API calls.**
The tradeoff: Mem0 can leverage LLM intelligence for memory extraction (useful when input is unstructured), while Surreal-Memory relies on its own extraction pipeline.
...
## Surreal-Memory vs Cognee — Competitive Benchmark
...
| Operation | Surreal-Memory | Cognee | Speedup |
|-----------|:------------:|:------:|:-------:|
...
Cognee's write bottleneck: every `add()` + `cognify()` cycle triggers multiple LLM calls for entity extraction, relationship mining, and knowledge graph construction. Surreal-Memory encodes directly into neural structures — no LLM needed.

### Accuracy

| Metric | Surreal-Memory | Cognee | Winner |
|--------|:------------:|:------:|:------:|
...
```
Surreal-Memory wins : 3 / 6 categories (speed × 3)
Cognee wins       : 3 / 6 categories (accuracy × 3)
```

**The tradeoff is clear**: Surreal-Memory dominates speed (80x write, 18x read, 48x conversation) and cost ($0 vs 149 API calls). Cognee wins on accuracy through LLM-powered knowledge graph construction — but at 80x the latency and significant per-operation cost.

For real-time AI agent workflows where sub-second response matters, Surreal-Memory is the clear choice. For offline knowledge base construction where accuracy is paramount and cost/latency are acceptable, Cognee's approach has merit.

### Reproduce

```bash
pip install cognee surreal-memory
...
### Overall (Surreal-Memory vs Naive Baseline)


| Metric | Surreal-Memory | Naive Baseline | Winner |
| --- | --- | --- | --- |
...
| Category | Surreal-Memory | Baseline | Count |
| --- | --- | --- | --- |
