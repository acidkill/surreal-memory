...
Surreal-Memory works **without embeddings** — its core retrieval uses spreading activation on a neural graph. Embeddings are an **optional enhancement** that adds cross-language recall and semantic discovery between unrelated memories.
...
The easiest way to enable embeddings — let Surreal-Memory detect what's available:
...
```bash
pip install surreal-memory[embeddings-openai]
```
...
```bash
pip install surreal-memory[embeddings-openrouter]
```
...
When you run `smem consolidate`, Surreal-Memory:
