...
Surreal-Memory automatically detects repetitive workflow patterns from your tool usage and materializes them as learned habits. This guide explains how habit detection works, what the output looks like, and how to tune it.
...
```bash
smem consolidate --strategy learn_habits
```
...
**Day 1-3**: You use Surreal-Memory normally. The system records action events:
...
- `smem_habits(action="suggest", current_action="recall")` → suggests "remember"
