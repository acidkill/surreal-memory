...
<div class="qs-steps">
  <div class="qs-step qs-reveal">
    ...
    <div class="qs-step__desc">
      <code>pip install surreal-memory[surrealdb]</code><br>
...
Run `smem doctor` to see if everything is healthy:
...
Something wrong? Run `smem doctor --fix` to auto-remediate hooks, dedup, and embedding issues.
...
<div class="qs-features qs-reveal">
  <div class="qs-feature">
    ...
    <div class="qs-feature__desc">
      ...
      <br><code>smem_remember("chose Redis over Memcached because...")</code>
    </div>
  </div>
  <div class="qs-feature">
    <div class="qs-feature__icon">🔍</div>
    ...
    <div class="qs-feature__desc">
      ...
      <br><code>smem_recall("caching strategy")</code>
    </div>
  </div>
  <div class="qs-feature">
    <div class="qs-feature__icon">🩺</div>
...
Without `--full`, run `smem init` for the basics, then `smem setup embeddings` separately.
...
This launches SurrealDB with the schema and vector index pre-configured. The community plugin (`surreal-memory[surrealdb]`) connects automatically.
...
```python
smem_recall("database migration strategy", depth=2)
```
...
```
smem_hypothesize("Redis is better than Memcached for our read pattern")
smem_evidence("mem_id", "supports", confidence=0.8)
smem_predict("Latency will drop 40% after Redis migration", deadline="2026-04-01")
smem_verify("prediction_id", outcome="correct")
```
...
```bash
smem serve -p 8765    # Start API server with auto-consolidation
```
...
crontab -e
0 3 * * * ~/.surrealmemory/maintenance.sh

# Windows — run via Task Scheduler
~\.surrealmemory\maintenance.ps1
```

## Need Help?

- **Issues?** Run `smem doctor --fix` first
- **Questions?** [GitHub Discussions](https://github.com/acidkill/surreal-memory/discussions)
- **Bugs?** [Open an issue](https://github.com/acidkill/surreal-memory/issues)
