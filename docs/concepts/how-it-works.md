...
```bash
smem remember "Met Alice at coffee shop to discuss API design, she suggested rate limiting"
```
...
```bash
smem recall "What did Alice suggest last Tuesday?"
```
...
When you request context (`smem_context`), Surreal-Memory doesn't just return the most recent memories. It uses a **5-factor composite scoring** system to select the most relevant items:
